...
namespace arbor {
    namespace engine {
        ...
        std::expected<void, std::string> renderer::pipeline::reload() {
            ...
            m_parent.m_logger->debug("creating a vulkan pipeline with {} new stages", m_changed_pipeline_stages.size());
            ...
            pipeline_create_info.stageCount          = m_changed_pipeline_stages.size();
            pipeline_create_info.pStages             = m_changed_pipeline_stages.data();
            ...
            m_changed_pipeline_stages.clear();
            return {};
        }
        ...
        std::expected<void, std::string> renderer::pipeline::bind_shader(const std::filesystem::path& glsl_source, shader::etype type) {
            ...
            m_changed_pipeline_stages.push_back({});
            auto& create_info = m_changed_pipeline_stages.back();
