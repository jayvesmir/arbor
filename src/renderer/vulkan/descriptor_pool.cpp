...
namespace arbor {
    namespace engine {
        std::expected<void, std::string> renderer::pipeline::make_vk_descriptor_pool_and_sets() {
            ...
            if (auto res = vkCreateDescriptorPool(m_parent.vk.device, &pool_create_info, nullptr, &m_descriptor_pool); res != VK_SUCCESS)
            ...
            if (auto res = vkAllocateDescriptorSets(m_parent.vk.device, &allocation_info, m_descriptor_sets.data()); res != VK_SUCCESS)
